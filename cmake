...
add_custom_target         (script COMMAND ${CMAKE_COMMAND} -E env bash ${CMAKE_SOURCE_DIR}/../A/deps.sh ${CMAKE_INSTALL_PREFIX} --deps ${CMAKE_SOURCE_DIR}/deps --debug \$(cat ${CMAKE_BINARY_DIR}/debug.txt) WORKING_DIRECTORY ${CMAKE_SOURCE_DIR})
...
target_include_directories(ether PRIVATE . ./src)
